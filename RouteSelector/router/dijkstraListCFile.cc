...
void dijkstra(struct Graph* graph, int src, int vehicle)
{
    ...
    double dist[V][4];      // dist values used to pick minimum weight edge in cut
    int trobat=0;
    ...
    for (int v = 0; v < V; ++v)
    {
        dist[v][vehicle]= INT_MAX;
        minHeap->array[v] = newMinHeapNode(v, dist[v][vehicle]);
        minHeap->pos[v] = v;
    }

    // Make dist value of src vertex as 0 so that it is extracted first
    minHeap->array[src] = newMinHeapNode(src, dist[src][vehicle]);
    minHeap->pos[src]   = src;
    dist[src][vehicle]= 0;
    decreaseKey(minHeap, src, dist[src][vehicle]);

    pred[src].cost[vehicle]=0.0;

    pred[src].vehicle[0]=0;
    pred[src].vehicle[1]=1;
    pred[src].vehicle[2]=2;
    pred[src].vehicle[3]=3;
    ...
    while (!isEmpty(minHeap))
    {
        ...
        while (pCrawl != NULL)
        {
            ...
            if (vehicle==0 && isInMinHeap(minHeap, v) && dist[u][0] != INT_MAX &&
                                          pCrawl->walkingCost + dist[u][0] < dist[v][0])
            {
                dist[v][0] = dist[u][0] + pCrawl->walkingCost;
                pred[v].pred[0] = u;
                pred[v].cost[0]=dist[u][0] +pCrawl->walkingCost;
            ...
            if (vehicle==1 && isInMinHeap(minHeap, v) && dist[u][1] != INT_MAX &&
                                          pCrawl->CarCost + dist[u][1] < dist[v][1])
            {
                dist[v][1] = dist[u][1] + pCrawl->CarCost;
                pred[v].pred[1] = u;
                pred[v].cost[1] = dist[u][1] +pCrawl->CarCost;
            ...
            if (vehicle==2 && isInMinHeap(minHeap, v) && dist[u][2] != INT_MAX &&
                                          pCrawl->BRPCost + dist[u][2] < dist[v][2])
            {
                dist[v][2] = dist[u][2] + pCrawl->BRPCost;
                pred[v].pred[2] = u;
                pred[v].cost[2] = dist[u][2] +pCrawl->BRPCost;
                ...
                // update distance value in min heap also
                decreaseKey(minHeap, v, dist[v][2]);
            }
            if (vehicle==2 && isInMinHeap(minHeap, v) && dist[u][2] != INT_MAX &&
                                          pCrawl->walkingCost + dist[u][0] < pred[v].cost[2]
                                          && pCrawl->walkingCost<pCrawl->BRPCost)
            {
                dist[v][2] = dist[u][0] + pCrawl->walkingCost;
                pred[v].pred[2] = u;
                pred[v].cost[2] = dist[u][0] +pCrawl->walkingCost;
                pred[v].vehicle[2]=pred[src].vehicle[0];
                pred[v].rampPos[2]=pCrawl->rampPos;
                pred[v].rampNeg[2]=pCrawl->rampNeg;
                pred[v].dist[2]=pCrawl->dist;
            ...
            if (vehicle==3 && isInMinHeap(minHeap, v) && dist[u][3] != INT_MAX &&
                                          pCrawl->AllTerrainCost + dist[u][3] < dist[v][3])
            {
                dist[v][3] = dist[u][3] + pCrawl->AllTerrainCost;
                pred[v].pred[3] = u;
                pred[v].cost[3]=dist[u][3] +pCrawl->AllTerrainCost;
                ...
                // update distance value in min heap also
                decreaseKey(minHeap, v, dist[v][3]);
            }
            if (vehicle==3 && isInMinHeap(minHeap, v) && dist[u][3] != INT_MAX &&
                                          pCrawl->walkingCost + dist[u][0] < pred[v].cost[3]
                                          && pCrawl->walkingCost<pCrawl->BRPCost)
            {
                dist[v][3] = dist[u][0] + pCrawl->walkingCost;
                pred[v].pred[3] = u;
                pred[v].cost[3] = dist[u][0] +pCrawl->walkingCost;
                pred[v].vehicle[3]=pred[src].vehicle[0];
                pred[v].rampPos[3]=pCrawl->rampPos;
                pred[v].rampNeg[3]=pCrawl->rampNeg;
                pred[v].dist[3]=pCrawl->dist;
            ...
            if (vehicle==1 && isInMinHeap(minHeap, v) && dist[u][1] != INT_MAX && vehicles[u][0] &&
                                          pCrawl->walkingCost + dist[u][1] < pred[v].cost[0] )
            {
                dist[v][0] = dist[u][1] + pCrawl->walkingCost;
                pred[v].pred[0] = u;
                pred[v].cost[0] = dist[u][1] +pCrawl->walkingCost;
                pred[v].vehicle[0]=pred[src].vehicle[1];
                pred[v].rampPos[0]=pCrawl->rampPos;
                pred[v].rampNeg[0]=pCrawl->rampNeg;
                pred[v].dist[0]=pCrawl->dist;
            ...
            if (vehicle==2 && isInMinHeap(minHeap, v) && dist[u][2] != INT_MAX && vehicles[u][1] &&
                                          pCrawl->walkingCost + dist[u][2] < pred[v].cost[0])
            {
                dist[v][0] = dist[u][2] + pCrawl->walkingCost;
                pred[v].pred[0] = u;
                pred[v].cost[0] = dist[u][2] +pCrawl->walkingCost;
                pred[v].vehicle[0]=pred[src].vehicle[2];
                pred[v].rampPos[0]=pCrawl->rampPos;
                pred[v].rampNeg[0]=pCrawl->rampNeg;
                pred[v].dist[0]=pCrawl->dist;
            ...
            if (vehicle==3 && isInMinHeap(minHeap, v) && dist[u][3] != INT_MAX && vehicles[u][2] &&
                                          pCrawl->walkingCost + dist[u][3] < pred[v].cost[0])
            {
                dist[v][0] = dist[u][3] + pCrawl->walkingCost;
                pred[v].pred[0] = u;
                pred[v].cost[0] = dist[u][3] +pCrawl->walkingCost;
                pred[v].vehicle[0]=pred[src].vehicle[3];
                pred[v].rampPos[0]=pCrawl->rampPos;
                pred[v].rampNeg[0]=pCrawl->rampNeg;
                pred[v].dist[0]=pCrawl->dist;
...
int *findPredecesor(int key,int vehicle){
    ...
    returnValue[2]=pred[key].vehicle[vehicle];
    return returnValue;
}
