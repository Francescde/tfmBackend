...
struct AdjListNode* newAdjListNode(int dest, double walkingCost,double CarCost,double BRPCost,double AllTerrainCost,double rampPos, double rampNeg, double dist)
{
    ...
    newNode->dest = dest;
    newNode->cost[0] = walkingCost;
    newNode->cost[1] = CarCost;
    newNode->cost[2] = BRPCost;
    newNode->cost[3] = AllTerrainCost;
...
void dijkstra(struct Graph* graph, int src)
{
    ...
    while (!isEmpty(minHeap))
    {
        ...
        while (pCrawl != NULL)
        {
            ...
            double costu = INT_MAX;
            int vehicle = 0;

            for (int ii =0; ii<4;++ii){
                //printf("calculate%f<%f\n",costu,pCrawl->cost[ii]);
                if(costu > pCrawl->cost[ii]){
                    costu=pCrawl->cost[ii];
                    vehicle=ii;
                }
            }
                                          ...
                                          costu + dist[u] < dist[v])
            {
                dist[v] = dist[u] + costu;
                pred[v].pred = u;
                pred[v].cost=dist[u] +costu;
                pred[v].data=pCrawl;
                pred[v].vehicle=vehicle;
                pred[v].visited=true;
...
extern "C" bool findVehicleCanGoToPred(int key,int vehicle);
extern "C" void solve(long long start, int vehicle);
extern "C" void inicializeEdge(int node1,int node2, double walkingCost,double CarCost,double BRPCost,double AllTerrainCost,double rampPos, double rampNeg, double dist);
...
void solve(long long start, int vehicle){
    ...
    dijkstra(globalGraph, start);
}

bool findVehicleCanGoToPred(int key,int vehicle){
    if(pred[key].visited)
        return pred[key].data->cost[vehicle]<9999;
    return false;
}
...
long long *findPredecesor(int key,int vehicle){
    ...
    if(pred[key].visited) {
        returnValue[0]=pred[key].pred;
        returnValue[1]=0;//nodes[pred[key].pred];
        returnValue[2]=pred[key].vehicle;
    }
    else{
        returnValue[0]=-1;
        returnValue[1]=-1;
...
double *findPredecesorValues(int key,int vehicle){
    ...
    if(pred[key].visited) {
        struct AdjListNode* pCrawl =  pred[key].data;
        returnValue[3]=pred[key].cost;
        returnValue[0]=pCrawl->rampPos;
        returnValue[1]=pCrawl->rampNeg;
        returnValue[2]=pCrawl->dist;
        returnValue[4]=pCrawl->cost[0];
        returnValue[5]=pCrawl->cost[1];
        returnValue[6]=pCrawl->cost[2];
        returnValue[7]=pCrawl->cost[3];
    }
    else{
        returnValue[3]=-1;//
        returnValue[0]=-1;//pCrawl->rampPos;
        returnValue[1]=-1;//pCrawl->rampNeg;
        returnValue[2]=-1;//pCrawl->dist;
        returnValue[4]=-1;//pCrawl->cost[0];
        returnValue[5]=-1;//pCrawl->cost[1];
        returnValue[6]=-1;//pCrawl->cost[2];
        returnValue[7]=-1;//pCrawl->cost[3];
    }
    return returnValue;
}
